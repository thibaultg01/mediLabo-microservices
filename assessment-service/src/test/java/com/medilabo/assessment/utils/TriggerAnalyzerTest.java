...
class TriggerAnalyzerTest {
	...
	void detects_basic_triggers_case_and_accents_ignored() {
		...
		List<NoteDto> notes = List.of(note("Hémoglobine A1C au-dessus du seuil."), note("microalbumine présente."),
				note("TAILLE correcte."), note("Poids égal ou inférieur au recommandé."));
		assertEquals(4, TriggerAnalyzer.countTriggers(notes));
	}

	@Test
	void anormal_matches_anormale_and_anormales_once() {
		List<NoteDto> notes = List.of(note("Audition anormale récemment."), note("Analyses anormales hier."));
		assertEquals(1, TriggerAnalyzer.countTriggers(notes));
	}

	@Test
	void cholesterol_with_accents_is_detected() {
		List<NoteDto> notes = List.of(note("Cholestérol total élevé."));
		assertEquals(1, TriggerAnalyzer.countTriggers(notes));
	}

	@Test
	void fumeur_and_fumeuse_are_counted_as_two_distinct_triggers_in_this_version() {
		List<NoteDto> notes = List.of(note("Le patient est fumeur."), note("La patiente précédente était fumeuse."));
